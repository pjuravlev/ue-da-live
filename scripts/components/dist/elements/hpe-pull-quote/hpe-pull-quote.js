 ...
 */
let HpePullQuote = class HpePullQuote extends LitElement {
    constructor() {
        super(...arguments);
        this.hideQuoteSign = false;
        this._hasAttribution = false;
    }
    _onAttributionSlotChange() {
        const attributionNameSlot = this.shadowRoot?.querySelector('slot[name="attribution-name"]');
        const attributionRoleSlot = this.shadowRoot?.querySelector('slot[name="attribution-role"]');
        const hasAttributionName = (attributionNameSlot?.assignedNodes({ flatten: true }).length ?? 0) > 0;
        const hasAttributionRole = (attributionRoleSlot?.assignedNodes({ flatten: true }).length ?? 0) > 0;
...
HpePullQuote.styles = styles;
__decorate([
    property({ type: Boolean, attribute: "hide-quote-sign", reflect: true })
], HpePullQuote.prototype, "hideQuoteSign", void 0);
__decorate([
    state()
], HpePullQuote.prototype, "_hasAttribution", void 0);
HpePullQuote = __decorate([
    customElement("hpe-pull-quote")
], HpePullQuote);
export { HpePullQuote };
//# sourceMappingURL=hpe-pull-quote.js.map